...
const getCity = () => {
  ...
  openWeatherCall(URL_API).then((data) => {
    ...
    if (data.name) {
      ...
      document.querySelector(".principaParams__city").innerHTML =
        "Ciudad inexistente";
    }
    ...
    document.querySelector(
      "#weatherIcon"
    ).src = `./public/icons/description_icons/${data.weather[0].icon}.png`;
  });
};
...
window.onload = () => {
  if ("geolocation" in navigator) {
    navigator.geolocation.getCurrentPosition((position) => {
      ...
      //api.openweathermap.org/data/2.5/weather?q=${city}&limit=5&appid=${API_KEY}&units=metric
      http: getCurrentWeather(URL_API);
    });
  } else {
    ...
    document.querySelector(
      "#weatherIcon"
    ).src = `../../public/assets/icons/description_icons/01d.png`;
    console.log(document.querySelector("#weatherIcon").src);
  }
